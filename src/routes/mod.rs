use std::error::Error;
use rocket::{fairing::{Fairing, Info, Kind}, http::Status, request::Outcome, response::status::Custom, Response};
use rocket_db_pools::Connection;
use serde_json::{json, Value};
use rocket::request::{Request, FromRequest};
use rocket_db_pools::deadpool_redis::redis::AsyncCommands;

use crate::{models::{RoleCode, User}, repositories::{RoleRepository, UserRepository}};
...
impl<'r> FromRequest<'r> for User {
    ...
    if let Some(header_value) = session_header {
        ...
        .expect("Cannot connect to redis in guard");

        let mut db = req.guard::<Connection<DbConn>>().await
        .expect("Cannot connect to Postgres in guard");
...
pub fn options(_route_args: Option<std::path::PathBuf>) {
    // Jus to add CORS headers via fairing.
...
impl<'r> FromRequest<'r> for EditorUser {
    ...
    async fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        if let Ok(roles) = RoleRepository::find_by_user(&mut db, &user).await {
            ...
            rocket::info!("User is editor: {:?}", is_editor);
