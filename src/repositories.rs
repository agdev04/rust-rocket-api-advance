...
impl RustaceanRepository {
    pub async fn all(c: &mut AsyncPgConnection, limit: i64) -> QueryResult<Vec<Rustacean>> {
...
impl  CrateRepository {
    pub async fn all(c: &mut AsyncPgConnection, limit: i64) -> QueryResult<Vec<Crate>> {
...
impl UserRepository {
    ...
    pub async fn find_with_roles(c: &mut AsyncPgConnection) -> QueryResult<Vec<(User, Vec<(UserRole, Role)>)>> {
        ...
        let result = users_roles::table.inner_join(roles::table).load::<(UserRole, Role)>(c).await?
        .grouped_by(&users);
        ...
        for role_code in role_codes {
            let new_user_role = {
                if let Ok(role) = RoleRepository::find_by_code(c, &role_code).await {
                    NewUsersRole { user_id: user.id, role_id: role.id}
                } else {
